...
def register_socket_handlers(socketio, users_db, channels, command_processor, livekit_manager):  # Added livekit_manager
    ...
    @socketio.on('send_dice_roll')
    def handle_send_dice_roll(data):
        request_id = data.get('request_id', str(uuid.uuid4()))
        if request_id in processed_requests:
            print(f"Skipping duplicate dice roll request: {request_id}")
            return
        processed_requests.add(request_id)
        ...
        if not channel or not roll_data or not message:
            emit('error', {'msg': 'Invalid dice roll data'})
            return
        
        sender = users_db[user_uuid]['username']
        ...
        try:
            ...
            c.execute("INSERT INTO messages (channel, sender, message, is_media, timestamp, replied_to) VALUES (?, ?, ?, ?, ?, ?)",
                      (channel, sender, message, 0, timestamp, None))
            message_id = c.lastrowid
            conn.commit()
            conn.close()
            
            print(f"Saved dice roll message ID {message_id} for {sender} in channel {channel}")
            
            # Broadcast dice roll to channel
            socketio.emit('receive_message', {
                'id': message_id,
                'channel': channel,
                'sender': sender,
            ...
            emit('error', {'msg': f'Failed to send dice roll: {str(e)}'})
            print(f"Error sending dice roll: {e}")
