...
async function addDiceRollMessage(sender, text, timestamp, messageId, rollData, repliedTo = null) {
    const messagesContainer = document.getElementById('messages-container');
    if (!messagesContainer) {
    ...
    messageGroup.className = 'message-group dice-roll-message';
    ...
    // Create dice roll display
    let rollDetails = '';
    if (rollData.count > 1) {
        rollDetails = `<div class="dice-roll-details">Individual rolls: [${rollData.rolls.join(', ')}]</div>`;
    }
    ...
    if (rollData.sides === 20 && rollData.count === 1) {
        if (rollData.rolls[0] === 20) {
            criticalIndicator = '<div class="critical-hit">🔥 CRITICAL HIT!</div>';
        } else if (rollData.rolls[0] === 1) {
            criticalIndicator = '<div class="critical-fail">💥 CRITICAL FAIL!</div>';
        }
    }
    
    const diceRollDisplay = `
        <div class="dice-roll-container">
            <div class="dice-roll-formula">${rollData.formula}</div>
            <div class="dice-roll-result">${rollData.total}</div>
            ${rollDetails}
            ${criticalIndicator}
        </div>
    `;
    ...
    messageGroup.innerHTML = `
        ...
        <div class="message-content">
            ...
            <div class="message-text dice-roll-text">${escapeHtml(text)}</div>
            ${diceRollDisplay}
    ...
    // Add event listeners for reactions, replies, and delete
