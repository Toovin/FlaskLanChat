...
socket.on('receive_message', (data) => {
    ...
    if (data.channel === currentChannel) {
        ...
        // Handle dice roll messages specially
        if (data.is_dice_roll && data.roll_data) {
            addDiceRollMessage(data.sender, data.message, data.timestamp, data.id, data.roll_data, data.replied_to);
        } else {
