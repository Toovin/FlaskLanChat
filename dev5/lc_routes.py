...
from lc_database import create_folder, get_folder_structure, get_folder_by_id, get_files_in_folder, add_file_to_folder, delete_folder, rename_folder
import os
...
def register_routes(app, socketio):
    ...
    def upload_file():
        ...
        for file in files:
            ...
            try:
                file.save(filepath)
                url = f'/static/{storage_dir}/{filename}'.replace('\\', '/')
                thumbnail_url = None
                     ...
                     try:
                         ...
                         if thumb_result:
                             if os.path.exists(thumbnail_filepath) and os.path.getsize(thumbnail_filepath) > 0:
                                 ...
                                 full_thumbnail_path = os.path.join('static', 'thumbnails', storage_dir, thumbnail_filename)
                ...
                # Save file to database if it's a file share upload
                if storage_dir == 'file_share_Uploads':
                    try:
                        add_file_to_folder(filename, filepath, folder_id, file_size, 'system')
                        print(f"File saved to database: {filename} in folder {folder_id}")
                    except Exception as db_error:
                        print(f"Failed to save file to database: {str(db_error)}")
