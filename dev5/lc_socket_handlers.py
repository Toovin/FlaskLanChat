...
from lc_database import save_user, get_user_by_username, update_user_avatar, load_messages, add_reaction, remove_reaction, get_reactions, get_standard_timestamp, update_user_settings, get_user_settings, create_channel, delete_channel, load_channels, get_city_data, recruit_unit, assign_villager_task
import sqlite3
import uuid
import threading
...
def register_socket_handlers(socketio, users_db, channels, command_processor, livekit_manager=None):
    users = {}  # sid -> user_uuid
    ...
    def handle_join_voice(data):
        if not livekit_manager:
            emit('voice_error', {'msg': 'Voice chat not configured'})
            return
        user_uuid = users.get(request.sid)
        if not user_uuid:
            emit('voice_error', {'msg': 'User not authenticated'})
            return
        channel = data.get('channel', '').strip()
        if not channel:
            emit('voice_error', {'msg': 'Invalid channel'})
            return
    ...
    def handle_send_message(data):
        ...
        timestamp = get_standard_timestamp()
        c.execute("INSERT INTO messages (channel, sender, message, is_media, timestamp, replied_to, image_url, thumbnail_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                  (channel, sender, message, is_media, timestamp, replied_to, image_url, thumbnail_url))
        message_id = c.lastrowid
        conn.commit()
        conn.close()
        print(f"Saved message ID {message_id} for {sender} in channel {channel}")
...
# ===== ADVENTURE SYSTEM SOCKET HANDLERS =====

    @socketio.on('get_city_data')
    def handle_get_city_data():
        user_uuid = users.get(request.sid)
        if not user_uuid:
        ...
        try:
            city_data = get_city_data(user_uuid)
            emit('city_data', {'city': city_data})
        except Exception as e:
            emit('error', {'msg': f'Failed to load city data: {str(e)}'})

    @socketio.on('recruit_unit')
    def handle_recruit_unit(data):
        user_uuid = users.get(request.sid)
        if not user_uuid:
            emit('error', {'msg': 'Not authenticated'})
            return
        ...
        try:
            result = recruit_unit(user_uuid, unit_type, unit_class)
            emit('recruitment_result', result)
        except Exception as e:
            emit('recruitment_result', {'success': False, 'message': f'Recruitment failed: {str(e)}'})

    @socketio.on('assign_villager_task')
    def handle_assign_villager_task(data):
        user_uuid = users.get(request.sid)
        if not user_uuid:
            emit('error', {'msg': 'Not authenticated'})
            return
        ...
        try:
            result = assign_villager_task(user_uuid, villager_id, task)
            emit('task_assignment_result', result)
        except Exception as e:
            emit('task_assignment_result', {'success': False, 'message': f'Task assignment failed: {str(e)}'})