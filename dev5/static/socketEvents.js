...
socket.on('receive_message', (data) => {
    ...
    if (data.channel === currentChannel) {
        ...
        if (data.is_media) {
            // Check if message contains JSON with multiple attachments
            ...
            if (typeof data.message === 'string') {
                try {
                    ...
                    if (parsed.attachments && Array.isArray(parsed.attachments) && parsed.attachments.length > 1) {
                        ...
                        messageData = data.message; // Keep original JSON for carousel
                    }
                } catch (e) {
            ...
            // Only create simple object for single attachments
