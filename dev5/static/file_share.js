...
function loadCurrentFolderFiles() {
    ...
    const isRootFolder = foldersData.length > 0 && currentFolderId === foldersData[0].id;

    const url = isRootFolder ?
        '/files?storage_dir=file_share_Uploads' :
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            files.forEach(file => {
                ...
                const lazyUrl = isRootFolder ?
                    `/lazy-file/file_share_Uploads/${encodeURIComponent(filename)}` :
                    `/lazy-file/${encodeURIComponent(file.filepath || filename)}`;
...
function handleFileUpload(e) {
    ...
    .then(data => {
        ...
        // Extract attachments with URLs and thumbnails
